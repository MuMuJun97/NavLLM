...
class MetaLoader:
    ...
    def __init__(
        ...
        self.names: List[str] = []
        ratios: List[int] = []
        for n, l in loaders.items(): # loaders ['mlm','mrc','sap','og']
...
def build_dataloader(task, dataset, collate_fn, is_train: bool, opts):
    ...
    batch_size = opts.train_batch_size if is_train else opts.val_batch_size # train_batch_size=2
    # if task == 'itm': batch_size = max(1, batch_size // 2)

    if opts.local_rank == -1: # single GPU training, local_rank=-1
        if is_train: #
            sampler: Union[
    ...
    loader = DataLoader(
        dataset,
        sampler=sampler, # training: RandomSampler.
        batch_size=batch_size, # 2
        num_workers=opts.n_workers, # 1
        pin_memory=opts.pin_mem, # True
        collate_fn=collate_fn, # mlm_collate
        drop_last=False,
    )
