...
def get_angle_fts(headings, elevations, angle_feat_size):
    """
    Features: VLN-DUET论文,构建朝向特征.
        The orientation feature [11] contains sin(·) and cos(·) values for heading and elevation angles.
    Args:
        headings:
        elevations:
        angle_feat_size:
            ...
            for i, item in enumerate(data):
                if item['included']:
                    for j, conn in enumerate(item['unobstructed']):
                        if conn and data[j]['included']:  # 第j个viewpoint与当前viewpoint(i)相连接,可以导航.
                            # 存储节点(image_id)的绝对坐标位置.
                            positions[item['image_id']] = np.array([item['pose'][3],
                                                                    item['pose'][7], item['pose'][11]]);
                            assert data[j]['unobstructed'][i], 'Graph should be undirected'

                            # 如果两个viewpoint是相连,可以导航的,则添加边. weight=两个节点之间的距离.
